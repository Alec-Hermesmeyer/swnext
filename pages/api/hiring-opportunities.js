...
import { canAccessHiringPipeline } from "@/lib/roles";
import { VALID_HIRING_STAGE_IDS } from "@/lib/hiring-pipeline";
...
export default async function handler(req, res) {
  ...
  if (!canAccessHiringPipeline(userContext.role)) {
    return res.status(403).json({ error: "You do not have access to the hiring pipeline" });
  }

  try {
    if (req.method === "GET") {
      const stageFilter = normalizeStage(req.query?.stage);
      let q = supabase.from("hiring_opportunities").select("*").order("created_at", { ascending: false });
      if (stageFilter) q = q.eq("stage", stageFilter);
      const { data, error } = await q;
      if (error) {
        console.error("hiring_opportunities GET:", error);
        return res.status(500).json({ error: error.message || "Could not load hiring pipeline" });
      }
    ...
    if (req.method === "POST") {
      ...
      const row = {
        title,
        applicant_name: String(body.applicant_name || "").trim() || null,
        contact_email: String(body.contact_email || "").trim() || null,
        contact_phone: String(body.contact_phone || "").trim() || null,
        position_applied: String(body.position_applied || "").trim() || null,
        stage,
      ...
      if (error) {
        console.error("hiring_opportunities POST:", error);
        return res.status(500).json({ error: error.message || "Could not create record" });
      }
    ...
    if (req.method === "PATCH") {
      ...
      const updates = {};
      if (body.applicant_name !== undefined) updates.applicant_name = String(body.applicant_name || "").trim() || null;
      ...
      if (body.position_applied !== undefined) updates.position_applied = String(body.position_applied || "").trim() || null;
      ...
      if (body.decline_reason !== undefined) updates.decline_reason = String(body.decline_reason || "").trim() || null;
      ...
      const { data, error } = await supabase
        ...
        .single();
      if (error) {
        console.error("hiring_opportunities PATCH:", error);
    ...
    if (req.method === "DELETE") {
      ...
      const { error } = await supabase.from("hiring_opportunities").delete().eq("id", id);
      if (error) {
        console.error("hiring_opportunities DELETE:", error);
        return res.status(500).json({ error: error.message || "Could not delete record" });
      }
    ...
    console.error("hiring-opportunities handler:", err);
