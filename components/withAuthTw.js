...
function getLoginTarget(asPath) {
  if (typeof asPath !== 'string') return '/login';
  if (!asPath.startsWith('/') || asPath.startsWith('//') || asPath === '/login') {
    return '/login';
  }
  ...
  const Wrapper = (props) => {
    ...
    useEffect(() => {
      if (!loading && !user && !isEmbedded) {
        router.replace(getLoginTarget(router.asPath));
      }
      ...
      return (
        <div className="min-h-screen flex items-center justify-center">
          <div className="flex items-center gap-3">
            ...
            <span className="text-neutral-600">Restoring session...</span>
          </div>
        </div>
      );
    }
