...
export default function TWLayoutEnhanced({ children, breadcrumbs = null, seoOptimized = true }) {
  const currentYear = new Date().getFullYear();
  
  // Enhanced footer structured data (only when SEO optimized)
  const footerStructuredData = seoOptimized ? {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white text-neutral-900">
      {/* Skip to main content link for accessibility (SEO enhanced) */}
      {seoOptimized && (
        <a 
          href="#main-content" 
          className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 bg-red-600 text-white px-4 py-2 rounded-md z-50 transition-all duration-200"
        >
      ...
      {/* Enhanced breadcrumbs with SEO markup */}
      {breadcrumbs && (
        <nav 
          ...
          <div className="container mx-auto px-4">
            <ol className="flex items-center space-x-2 text-sm" role="list">
              <li>
                <Link 
                  href="/" 
                  className="text-gray-500 hover:text-red-600 focus:outline-none focus:underline transition-colors duration-200"
      ...
      <main 
        id={seoOptimized ? "main-content" : undefined}
      ...
      {/* Enhanced footer with better SEO and accessibility */}
      <footer 
      ...
      >
        <div className="mx-auto w-full px-0">
          <div className="grid grid-cols-1 gap-8 px-6 py-12 md:grid-cols-4 md:px-10">
            ...
            {/* Company Logo and Description - Enhanced */}
            <div className="md:col-span-1">
              <div className="inline-flex h-20 w-20 items-center justify-center rounded-full bg-white mb-4">
                <Image
                  src="/swlogorwb.png"
                  alt={seoOptimized ? "S&W Foundation Contractors - Commercial Pier Drilling Company Logo" : "S&W"}
                  width={90}
                  height={26}
                  sizes="90px"
                  priority={false}
            ...
            <div>
              <h3 className="font-black mb-4">{seoOptimized ? "Company" : "Company"}</h3>
            ...
            {/* Services Links - Enhanced for SEO */}
            {seoOptimized && (
              <div>
            ...
            <div className={seoOptimized ? "" : "md:col-start-4"}>
              <h3 className="font-black mb-4">Contact{seoOptimized ? " Information" : ""}</h3>
              {seoOptimized ? (
              ...
              <div className="mt-6">
                {seoOptimized && <h4 className="font-semibold text-white mb-3">Follow Us</h4>}
                <div className="flex space-x-4">
                  <Link
                    ...
                    aria-label={seoOptimized ? "Visit S&W Foundation Contractors on Facebook (opens in new window)" : "Facebook"}
                  >
                    <FaFacebook size={seoOptimized ? 20 : 24} />
                  </Link>
                  <Link
                    ...
                    rel="noopener noreferrer"
                    className="text-neutral-200 hover:text-red-400 focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-offset-2 focus:ring-offset-[#0b2a5a] rounded transition-colors duration-200"
                    aria-label={seoOptimized ? "Visit S&W Foundation Contractors on LinkedIn (opens in new window)" : "LinkedIn"}
                  >
                    <FaLinkedin size={seoOptimized ? 20 : 24} />
                  </Link>
                </div>
              </div>
            </div>
          ...
          <div className="border-t border-white/10 px-6 py-6 md:px-10">
            <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
              <p className="text-sm text-neutral-200">
                © {currentYear} S&W Foundation Contractors{seoOptimized ? " Inc. All rights reserved." : ""}
              </p>
              {seoOptimized && (
        ...
        {/* Footer Structured Data - Only when SEO optimized */}
        {seoOptimized && footerStructuredData && (
          <script
