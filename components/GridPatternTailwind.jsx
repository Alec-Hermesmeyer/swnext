...
function Block({ x, y, fill = '#d1d5db', opacity = 0.3, ...props }) {
  return (
    <motion.path
      transform={`translate(${-2 * y + 96 * x} ${160 * y})`}
...
export function GridPatternTailwind({ 
  yOffset = 0, 
  interactive = false, 
  className = "",
  patternStroke = "#e5e7eb",
  patternOpacity = 1,
  blockFill = "#d1d5db",
  blockOpacity = 0.3,
  hoveredBlockFill = "#94a3b8",
  hoveredBlockOpacity = 0.45,
  ...props 
}) {
  let id = useId()
  let ref = useRef(null)
  ...
  return (
    <svg 
      ref={ref} 
      aria-hidden="true" 
      className={`relative left-0 right-0 h-[50vh] w-full ${className}`}
      {...props}
    >
      <rect width="100%" height="100%" fill={`url(#${id})`} strokeWidth="0" />
      <svg x="50%" y="1%" strokeWidth="0" className="overflow-visible">
        ...
        {hoveredBlocks.map((block) => (
          <Block
            ...
            x={block[0]}
            y={block[1]}
            fill={hoveredBlockFill}
            opacity={hoveredBlockOpacity}
