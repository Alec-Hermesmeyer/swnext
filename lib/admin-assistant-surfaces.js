...
export function buildScheduleOverviewForDates(targetDates, data) {
  ...
  return {
    ...
    quickActions: targetDates.length === 1
      ? [
          { label: "Add more workers", message: `Who else should I add to the schedule for ${targetDates[0]}?` },
          { label: "Set rig details", message: `Set the superintendents and trucks for ${targetDates[0]}` },
          { label: "Finalize", message: `Finalize the schedule for ${targetDates[0]}` },
          { label: "Send email", message: `Send the schedule email for ${targetDates[0]}` },
        ]
      : [
          { label: "Add more workers", message: "Who else should I add to the schedule?" },
          { label: "Finalize", message: `Finalize the schedule for ${targetDates[0]}` },
        ],
  };
}
...
export function buildScheduleBuilderContextSurface(message, data) {
  ...
  quickActions.push({
    ...
    message: `I'll build ${targetDate} from scratch. Show me the rigs and I'll tell you who goes where.`,
  });
  if (existingRigs.length) {
    quickActions.push({
      label: "Show current state",
      message: `What is scheduled for ${targetDate}?`,
    });
  }
  ...
  return {
    ...
    description: "Your crew and active jobs are listed below. Tell me who goes on which rig, or tap a quick action to get started.",
    readOnly: true,
    targetDate,
    ...
    quickActions,
    tips: [
      "Say something like 'Put John and Mike on Rig 1 for North Shore Pier'",
      "You can assign multiple workers in one message.",
      "Say 'done' or 'finalize' when the schedule looks right.",
    ],
  };
}
...
export function buildAssistantSurface({
  ...
  const wantsScheduleOverview =
    includesAny(text, [
      ...
      "packet",
      "packets",
    ]) && !wantsCrewCreate && !wantsCrewUpdate && !wantsScheduleBuild;
