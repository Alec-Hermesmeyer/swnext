...
export function buildSalesPipelineListSurface(data, options = {}) {
  ...
  return {
    ...
    description: demoMode
      ? "There are no opportunities in the database yet. Below are sample rows so you can see how the board will look. Add a real one with the quick action or open the full Sales page."
      : "Pre-award opportunities you can see. Open the full page to edit stages, values, and owners.",
    readOnly: true,
    demoMode,
    summary: [
    ...
    quickActions: [
      ...(writeAccessEnabled
        ...
        : []),
      { label: "Open full pipeline", href: "/admin/sales" },
    ],
    tips: demoMode
      ? [
          "Preview rows are not saved. Submit a real opportunity or use the full Sales page to manage deals.",
      ...
      : [
          ...
          "Use the full Sales page to move stages and edit every field.",
        ],
  };
}

function buildSalesOpportunityCreateSurface(prefill = {}) {
  const stageOptions = SALES_PIPELINE_STAGES.map((s) => ({
    label: s.label,
    value: s.id,
    hint: s.hint,
  }));
  return {
    id: createSurfaceId("sales_opportunity_create"),
    type: "sales_opportunity_create",
    module: "Sales",
    stage: "Intake",
    title: "Add a sales opportunity",
    description:
      "Capture a pre-award deal. This writes to the same pipeline as the Sales page.",
    ...
    fields: [
      ...
      },
      {
        ...
        type: "textarea",
        span: 2,
        placeholder: "Scope, competitors, risks…",
      },
    ],
    quickActions: [{ label: "Open full pipeline", href: "/admin/sales" }],
    tips: ["Only the title is required. You can refine details later on the Sales page."],
  };
}
