...
// Last two digits of the current calendar year, used as the implicit prefix
// when a handwritten job number omits the year (e.g., "0055" → "26-0055" in
// 2026). Without this, a bare "0055" can match an older job like "25-0055"
// because endsWith() is lenient.
...
function exactJobNumberMatch(jobs, raw) {
  ...
  const needle = trimmed.replace(/[^0-9]/g, "");
  if (!needle || needle.length < 3) return null;

  const hasYearPrefix = /^\d{2}[-/]/.test(trimmed);

  // No year prefix on the photo → assume current year. Try an exact match
  // against currentYear+needle first; only fall back to endsWith if no
  // current-year job exists with that suffix.
  if (!hasYearPrefix) {
    const yearNeedle = currentYearPrefix() + needle;
    const yearMatch = jobs.find((j) => {
      const jn = norm(j.job_number).replace(/[^0-9]/g, "");
      return jn === yearNeedle;
    });
...
// ---------------------------------------------------------------------------
// Keyword-based category matching (for equipment, crane, shop, etc.)
// ---------------------------------------------------------------------------
const KEYWORD_CATEGORY_MAP = [
  { pattern: /crane|peach/i, names: ["crane"] },
  { pattern: /bobcat|skytrak|jcb|icm|forklift|equipment/i, names: ["equipment"] },
  { pattern: /shop|yard/i, names: ["shop"] },
];

function keywordCategoryMatch(rigName, categories) {
  const text = norm(rigName);
  for (const { pattern, names } of KEYWORD_CATEGORY_MAP) {
    if (pattern.test(text)) {
      const cat = categories.find((c) => names.some((n) => norm(c.name).includes(n)));
      if (cat) return cat;
    }
  }
  return null;
...
// ---------------------------------------------------------------------------
// Color-based category assignment — fallback when fuzzy matching fails.
//
// The handwritten schedule groups rigs by highlight color (orange, yellow).
// We map each unique color to a category in sort order. This lets rows like
// "E6-40 Mighty Mouse" (which will never fuzzy-match "Rig 1") still land
// in the right category based on their visual grouping.
// ---------------------------------------------------------------------------
function assignCategoriesByColor(matchedRows, categories) {
  if (!categories || categories.length === 0) return;

  // Separate categories into "rig" categories (generic like Rig 1) and special ones
  const specialNames = new Set(["crane", "equipment", "shop"]);
  const rigCategories = categories
    .filter((c) => !specialNames.has(norm(c.name)))
    .sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));

  // Discover color groups in row order (first color seen → first rig category)
  const colorOrder = [];
  for (const row of matchedRows) {
    const color = row.highlight_color || "none";
    if (color !== "none" && !colorOrder.includes(color)) {
      colorOrder.push(color);
    }
  }

  // Build color → category map
  const colorToCat = {};
  colorOrder.forEach((color, i) => {
    if (i < rigCategories.length) {
      colorToCat[color] = rigCategories[i];
    }
  });

  // Assign categories to rows that have no match
  for (const row of matchedRows) {
    if (row.category?.match?.id) continue; // already matched by fuzzy

    // Try keyword match first (crane, equipment, shop)
    const kwMatch = keywordCategoryMatch(row.raw?.rig_name || "", categories);
    if (kwMatch) {
      row.category = {
        match: kwMatch,
        score: 0.7,
        confidence: CONFIDENCE.MEDIUM,
        candidates: [{ item: kwMatch, score: 0.7, confidence: CONFIDENCE.MEDIUM }],
        auto_assigned: "keyword",
      };
      continue;
    }
    ...
    const color = row.highlight_color || "none";
    const colorCat = colorToCat[color];
    if (colorCat) {
      row.category = {
        match: colorCat,
        score: 0.5,
        confidence: CONFIDENCE.MEDIUM,
        candidates: rigCategories.map((c) => ({
          item: c,
          score: c.id === colorCat.id ? 0.5 : 0.3,
          confidence: CONFIDENCE.MEDIUM,
        })),
        auto_assigned: "color",
      };
    ...
    // Last resort: assign to the first rig category
    if (rigCategories.length > 0) {
      row.category = {
        match: rigCategories[0],
        score: 0.3,
...
export function matchAllRows(visionOutput, matchers) {
  ...
  const matchedRows = rows.map((row) => matchRow(row, matchers));

  // Post-process: assign categories to rows that fuzzy matching couldn't resolve
  assignCategoriesByColor(matchedRows, matchers._raw?.categories || []);
