...
/** Pre-award sales pipeline (admin / ops — not the sales *role* chat tools list) */
const SALES_TOOLS = ["create_sales_opportunity"];
...
const OPERATIONS_PERMISSIONS = {
  ...
  chatTools: [...SCHEDULE_TOOLS, ...CAREER_TOOLS, ...CONTACT_TOOLS, ...SPAM_TOOLS, ...IMAGE_TOOLS, ...SALES_TOOLS, ...RAG_TOOLS],
  canWrite: true,
  dataModules: ["schedule", "careers", "contacts", "submissions", "sales"],
};

export const ROLE_PERMISSIONS = {
  admin: {
    pages: ALL_NAV.map((n) => n.href),
    chatTools: [...SCHEDULE_TOOLS, ...SOCIAL_TOOLS, ...CAREER_TOOLS, ...CONTACT_TOOLS, ...SPAM_TOOLS, ...IMAGE_TOOLS, ...SALES_TOOLS, ...RAG_TOOLS, ...ADMIN_TOOLS],
    canWrite: true,
