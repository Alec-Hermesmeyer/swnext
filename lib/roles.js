...
// Sidebar ordering mirrors the business lifecycle: Start → Win Work → Run Work
// → Money → People → Marketing → Admin. Each section is an intake → action →
// feedback loop that a new hire can read top-down to understand the shop.
const ALL_NAV = [
  // ── Start ──
  { href: "/admin", label: "AI Assistant", section: "Start" },
  { href: "/admin/dashboard", label: "Dashboard", section: "Start" },

  // ── Win Work (lead → bid → award → schedule request) ──
  { href: "/admin/contact", label: "Submissions", section: "Win Work" },
  { href: "/admin/sales", label: "Sales", section: "Win Work" },
  { href: "/admin/schedule-requests", label: "Schedule Requests", section: "Win Work" },
  { href: "/admin/client-portal", label: "Client Portal", section: "Win Work" },

  // ── Run Work (jobs → plan → broadcast → feedback) ──
  { href: "/admin/jobs", label: "Jobs", section: "Run Work" },
  { href: "/admin/crew", label: "Crew", section: "Run Work" },
  { href: "/admin/cranes", label: "Cranes", section: "Run Work" },
  { href: "/admin/crew-scheduler", label: "Crew Scheduler", section: "Run Work" },
  { href: "/admin/daily-board", label: "Daily Board", section: "Run Work" },
  { href: "/admin/field-reports", label: "Field Reports", section: "Run Work" },

  // ── Money ──
  { href: "/admin/job-costs", label: "Job Costs", section: "Money" },
  { href: "/admin/change-orders", label: "Change Orders", section: "Money" },
  { href: "/admin/revenue-reports", label: "Revenue Reports", section: "Money" },

  // ── People ──
  { href: "/admin/certifications", label: "Certifications", section: "People" },
  { href: "/admin/hiring", label: "Hiring", section: "People" },
  { href: "/admin/careers", label: "Careers", section: "People" },

  // ── Marketing ──
  { href: "/admin/blog", label: "Blog", section: "Marketing" },
  { href: "/admin/social-media", label: "Social Media", section: "Marketing" },
  { href: "/admin/image-assignments", label: "Page Images", section: "Marketing" },
  { href: "/admin/gallery", label: "Gallery", section: "Marketing" },

  // ── Admin ──
  { href: "/admin/company-contacts", label: "Contacts", section: "Admin" },
  { href: "/admin/knowledge-base", label: "Knowledge Base", section: "Admin" },
  { href: "/admin/team-insights", label: "Team Insights", section: "Admin" },
...
const BIDDING_TOOLS = ["analyze_bid", "add_competitor_intel", "get_bid_performance", "add_bid_details"];
...
const OPERATIONS_PERMISSIONS = {
  // Ops (incl. VP of Pre-Construction) lives through the full lifecycle:
  // win → run → money, plus the people side since they also drive hiring
  // decisions and need cert visibility for scheduling.
  pages: [
    // Start
    "/admin",
    "/admin/dashboard",
...
export const ROLE_PERMISSIONS = {
  ...
  safety: {
    // Safety mirrors Operations (full lifecycle visibility) AND owns the
    // public-facing photo audit: Gallery + Page Images so they can verify
    // everything published meets safety standards.
    ...OPERATIONS_PERMISSIONS,
    pages: [
      ...OPERATIONS_PERMISSIONS.pages,
      "/admin/gallery",
      "/admin/image-assignments",
    ],
  },
  social_media: {
    // Start + Marketing only — their domain is outbound content.
    pages: [
      "/admin",
      "/admin/dashboard",
      "/admin/blog",
  ...
  hr: {
    // Start + Run Work (crew roster/visibility) + People (manage).
    pages: [
      "/admin",
      "/admin/dashboard",
      "/admin/jobs",
  ...
  sales: {
    // Start + full Win Work (incl. schedule requests) + Money.
    pages: [
      "/admin",
      "/admin/dashboard",
      "/admin/contact",
  ...
  viewer: {
    // TV / view-only: AI chat + Daily Board only.
    pages: ["/admin", "/admin/daily-board"],
    chatTools: [...RAG_TOOLS],
    canWrite: false,
    dataModules: [],
  },
...
// Returns [{ section: "Start", links: [...] }, ...] in NAV_SECTION_ORDER,
// dropping sections that have no visible links for this role.
export function getVisibleNavSections(role, accessLevel) {
  const visible = getVisibleNavLinks(role, accessLevel);
  const bySection = new Map();
  visible.forEach((link) => {
    const key = link.section || "Other";
    if (!bySection.has(key)) bySection.set(key, []);
    bySection.get(key).push(link);
  });
  const ordered = [];
  NAV_SECTION_ORDER.forEach((name) => {
    if (bySection.has(name)) ordered.push({ section: name, links: bySection.get(name) });
  });
  // Any leftover sections not in the ordered list (future-proofing)
  Array.from(bySection.keys()).forEach((name) => {
    if (!NAV_SECTION_ORDER.includes(name)) {
      ordered.push({ section: name, links: bySection.get(name) });
    }
  });
  return ordered;
}
