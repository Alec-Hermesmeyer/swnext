...
const RECENT_BIDS_QUERY_RE =
  /\b(recent|our)\s+bids?\b|bid\s+pipeline|rfq|recent\s+bid\s+proposals|bid\s+opportunities/i;

/**
 * Format an array of document rows into a labelled result block.
 * Shared by all branches of the bidding fallback to eliminate duplication.
 */
function formatBidFallbackLines(rows, label, maxChars = 720) {
  // Deduplicate by filename — surface diverse docs, not 5 chunks from one file
  const seen = new Map();
  const deduped = [];
  for (const d of rows) {
    const meta = d.metadata && typeof d.metadata === "object" ? d.metadata : {};
    const key = meta.filename || meta.bid_document_id || d.id;
    const count = seen.get(key) || 0;
    if (count >= 2 && deduped.length >= 4) continue;
    seen.set(key, count + 1);
    deduped.push(d);
  }
  const lines = deduped.map((d, i) => {
    const meta = d.metadata && typeof d.metadata === "object" ? d.metadata : {};
    const fname = meta.filename || "";
    const section = meta.section ? ` (${meta.section})` : "";
    const snip = (d.content || "").replace(/\s+/g, " ").trim().slice(0, maxChars);
    return `[${label} ${i + 1}${fname ? ` | ${fname}` : ""}${section}]\n${snip}${(d.content || "").length > maxChars ? "…" : ""}`;
  });
  return lines.join("\n\n---\n\n");
}

/**
 * Fetch recent bidding rows as a fallback.  Used by multiple code paths.
 */
async function fetchRecentBiddingRows(supabase, limit = 14) {
  const { data, error } = await supabase
    ...
    .limit(limit);
  return (!error && data?.length) ? data : null;
}

/**
 * Bid chunks are dense technical text; generic questions ("recent bids") rarely
 * exceed high cosine similarity. Fall back to keyword OR newest bidding rows.
 */
async function fetchBiddingDocumentsFallback(supabase, query, options = {}) {
  const { recentListOnly = false } = options;

  // Path 1: user asked for "recent bids" or similar — just show newest
  if (recentListOnly || RECENT_BIDS_QUERY_RE.test(String(query || ""))) {
    const rows = await fetchRecentBiddingRows(supabase);
    if (!rows) return null;
    return {
      success: true,
      message: `Semantic search didn't line up tight on that phrasing — here are our **newest** embedded bid chunks (same stuff you'd scroll in Knowledge Base):\n\n${formatBidFallbackLines(rows, "Bid document")}`,
    };
  }

  // Path 2: extract keywords and try a keyword OR search
  const tokens = String(query || "")
    .split(/[^a-zA-Z0-9]+/)
    .map((t) => t.trim())
    .filter((t) => /^[a-zA-Z0-9]{3,48}$/.test(t))
    .filter((t, i, arr) => arr.indexOf(t) === i)
    .slice(0, 6);

  if (!tokens.length) {
    const rows = await fetchRecentBiddingRows(supabase, 10);
    if (!rows) return null;
    return {
      success: true,
      message: `Search was fuzzy — pulling **recent** bid embeddings so you still have something to work from:\n\n${formatBidFallbackLines(rows, "Bid document")}`,
    };
  }

  const orParts = tokens.map((t) => `content.ilike.%${t}%`).join(",");
  const { data, error } = await supabase
    .from("documents")
  ...
  if (error || !data?.length) {
    // Keywords missed — fall back to recency
    const rows = await fetchRecentBiddingRows(supabase, 10);
    if (!rows) return null;
    return {
      success: true,
      message: `No straight keyword hit on ${tokens.join(", ")} — still surfacing **latest** bid chunks in case the wording in the file differs:\n\n${formatBidFallbackLines(rows, "Bid document")}`,
    };
  }

  return {
    success: true,
    message: `Embedding match was soft — here are **keyword hits** in our bid docs (${tokens.join(", ")}):\n\n${formatBidFallbackLines(data, "Keyword")}`,
  };
}
...
export async function executeAdminAssistantMutation(supabase, name, args, context = {}) {
  switch (name) {
    ...
    case "search_knowledge_base": {
      ...
      const categoryFocus = String(args.category_focus || "").trim().toLowerCase();
      const biddingFocus = categoryFocus === "bidding";
      const sectionFilter = String(args.section_filter || "").trim().toLowerCase();

      // ── Role-based RAG interaction ──────────────────────────────────
      //
      // The caller's role (passed via context.userRole) determines:
      //  1. Query prefix — steers the embedding toward role-relevant content
      //  2. Category affinity — boosts similarity scores for role-relevant
      //     document categories so they rank higher in the final results
      //  3. Over-fetch multiplier — roles with affinity pull more candidates
      //     so the re-ranker has more material to work with
      //
      // Three role clusters:
      //  sales  → bidding, project_history, client_inquiry
      //  ops    → project_history, hiring, team_insights, company_info
      //  admin  → all categories equally (no boost)
      //
      const userRole = String(context?.userRole || "").trim().toLowerCase();

      // Category affinity: role → categories that get a similarity boost.
      // The boost (0.03–0.06) is small enough to never override a genuinely
      // better semantic match, but large enough to break ties in favour of
      // the role's domain when two results are semantically close.
      const ROLE_CATEGORY_AFFINITY = {
        sales:        { bidding: 0.06, project_history: 0.04, client_inquiry: 0.04 },
        operations:   { project_history: 0.05, hiring: 0.04, team_insights: 0.03, company_info: 0.03 },
        safety:       { project_history: 0.05, hiring: 0.04, team_insights: 0.03, company_info: 0.03 },
        hr:           { hiring: 0.06, team_insights: 0.05, company_info: 0.03 },
        social_media: { company_info: 0.04, project_history: 0.03 },
        // admin, owner, viewer: no affinity — neutral across all categories
      };
      const affinity = ROLE_CATEGORY_AFFINITY[userRole] || {};
      const hasAffinity = Object.keys(affinity).length > 0;

      // Role-aware query prefix for short queries
      const ROLE_QUERY_PREFIX = {
        sales:      "S&W Foundation Contractors sales bid proposal: ",
        operations: "S&W Foundation Contractors construction operations: ",
        safety:     "S&W Foundation Contractors safety operations: ",
        hr:         "S&W Foundation Contractors hiring team: ",
      };

      try {
        const { getEmbedding } = await import("@/lib/embeddings");

        // Pick the best domain prefix for this role + category focus combo
        let embeddingInput = query;
        if (query.length < 60) {
          let prefix;
          if (biddingFocus) {
            prefix = "S&W Foundation Contractors bid proposal: ";
          } else if (ROLE_QUERY_PREFIX[userRole]) {
            prefix = ROLE_QUERY_PREFIX[userRole];
          } else {
            prefix = "S&W Foundation Contractors: ";
          }
          embeddingInput = prefix + query;
        }

        const embedding = await getEmbedding(embeddingInput);

        const RAG_MATCH_THRESHOLD = parseFloat(process.env.RAG_MATCH_THRESHOLD) || 0.68;
        const RAG_BIDDING_POST_THRESHOLD =
          parseFloat(process.env.RAG_BIDDING_POST_THRESHOLD) || 0.48;
        const RAG_BIDDING_RPC_THRESHOLD =
          parseFloat(process.env.RAG_BIDDING_RPC_THRESHOLD) || 0.32;
        const RAG_MATCH_COUNT = parseInt(process.env.RAG_MATCH_COUNT, 10) || 8;
        const RAG_MAX_CHUNK_CHARS = parseInt(process.env.RAG_MAX_CHUNK_CHARS, 10) || 1200;

        const rpcThreshold = biddingFocus
          ? RAG_BIDDING_RPC_THRESHOLD
          : Math.max(RAG_MATCH_THRESHOLD - 0.05, 0.55);
        const postThreshold = biddingFocus ? RAG_BIDDING_POST_THRESHOLD : RAG_MATCH_THRESHOLD;
        // Over-fetch when we have role affinity — more candidates gives the
        // re-ranker better coverage of the role's preferred categories.
        const affinityMultiplier = hasAffinity && !biddingFocus ? 2 : 1;
        const matchCount = biddingFocus
          ? Math.max(RAG_MATCH_COUNT * 5, 40)
          : RAG_MATCH_COUNT * affinityMultiplier;
        ...
        if (!data?.length) {
          if (biddingFocus) {
            const fb = await fetchBiddingDocumentsFallback(supabase, query, {
              recentListOnly: RECENT_BIDS_QUERY_RE.test(query),
            });
            if (fb) return fb;
          }
          return {
            success: true,
            message: biddingFocus
              ? "No bid proposal rows in the database yet. Run Knowledge Base → sync “Bid proposals (sales)” after uploads."
              : "No relevant documents found in the knowledge base for that query.",
          };
        }

        // Apply role-based affinity boost BEFORE threshold filtering.
        // The boost is additive — e.g. a 0.65-scoring bidding doc becomes
        // 0.71 for a sales user, pushing it above the 0.68 threshold
        // when it would otherwise be excluded.
        const boosted = data.map((d) => {
          const cat = d.category || "general";
          const boost = affinity[cat] || 0;
          return {
            ...d,
            _rawSimilarity: d.similarity,
            similarity: Math.min(d.similarity + boost, 1.0),
          };
        });

        let filtered = boosted
          .filter((d) => d.similarity >= postThreshold)
          .sort((a, b) => b.similarity - a.similarity);

        // Optional section filter — narrow to a specific bid section type
        // (e.g. "exclusions", "scope_items", "priced_items", "assumptions", "raw_text")
        if (sectionFilter) {
          const sectionHits = filtered.filter(
            (d) => d.metadata && String(d.metadata.section || "").toLowerCase() === sectionFilter
          );
          if (sectionHits.length) {
            filtered = sectionHits;
          }
          // If no section-specific hits, keep the full set as fallback
        }

        let preamble = "";
        if (biddingFocus) {
          const bidRows = filtered.filter((d) => d.category === "bidding");
          if (bidRows.length) {
            filtered = bidRows;
          } else if (filtered.length) {
            preamble =
              "Tight **bidding**-only vector hits were thin — below are the best matches across categories (still worth scanning). For a cleaner read on our proposal, try a sharper query (GC name, job title, or doc name).\n\n";
          }
        }

        filtered = filtered.slice(0, 8);

        // Deduplicate: keep the best-scoring chunk per source document so
        // adjacent overlapping chunks from one file don't consume every slot.
        // Allow up to 2 chunks from the same document before skipping.
        const _seenDocKeys = new Map();
        const _deduplicated = [];
        for (const d of filtered) {
          const docKey =
            (d.metadata && (d.metadata.bid_document_id || d.metadata.filename)) || d.id;
          const count = _seenDocKeys.get(docKey) || 0;
          if (count >= 2 && _deduplicated.length >= 3) continue;
          _seenDocKeys.set(docKey, count + 1);
          _deduplicated.push(d);
        }
        filtered = (_deduplicated.length ? _deduplicated : filtered).slice(0, 5);
        ...
        if (!filtered.length) {
          ...
          return {
            success: true,
            message: biddingFocus
              ? "No bid chunks scored above the similarity threshold. Try syncing bid proposals on the Knowledge Base page, or ask using a distinctive project or client name."
              : "No sufficiently relevant documents found. The knowledge base may not have content on this topic yet.",
          };
        }

        const lines = filtered.map((d, i) => {
          // Display raw similarity (before role boost) so the LLM's
          // confidence assessment reflects actual semantic match quality.
          const rawScore = Math.round((d._rawSimilarity || d.similarity || 0) * 100);
          const category = d.category || "general";
          const meta = d.metadata && typeof d.metadata === "object" ? d.metadata : {};
          const section = meta.section ? ` | section: ${meta.section}` : "";
          const filename = meta.filename ? ` | file: ${meta.filename}` : "";
          const source = d.source ? ` | source: ${d.source}` : "";

          // Confidence band helps the model weigh results appropriately:
          //  HIGH   (≥80%) — strong match, safe to quote directly
          //  MEDIUM (≥65%) — likely relevant, verify before citing as fact
          //  LOW    (<65%) — supplementary context only
          const confidence =
            rawScore >= 80 ? "HIGH" : rawScore >= 65 ? "MEDIUM" : "LOW";

          // Detect structured data — JSON-like content from DB backfills
          // or uploads that start with [Source: …] headers followed by JSON.
          let content = d.content || "";
          const stripped = content.replace(/^\[Source:[^\]]*\]\s*/i, "").trimStart();
          const isStructured =
            stripped.startsWith("{") ||
            stripped.startsWith("[") ||
            (source.includes("backfill") && /^\w+:/.test(stripped));

          // Tighter budget for structured data prevents the LLM from
          // echoing raw field/value noise. Narrative text gets full budget.
          const charBudget = isStructured
            ? Math.min(RAG_MAX_CHUNK_CHARS, 800)
            : RAG_MAX_CHUNK_CHARS;
          if (content.length > charBudget) {
            // Try to cut at a sentence boundary to avoid mid-sentence truncation
            const cutPoint = content.lastIndexOf(". ", charBudget);
            content = cutPoint > charBudget * 0.6
              ? content.substring(0, cutPoint + 1) + " …"
              : content.substring(0, charBudget) + "…";
          }

          // For structured data, add a hint so the model summarises in
          // natural language instead of regurgitating key/value pairs.
          const structHint = isStructured ? " [structured data — summarize in plain English]" : "";

          return `[Result ${i + 1} | ${category} | ${rawScore}% match (${confidence})${section}${filename}${source}${structHint}]\n${content}`;
        });

        return {
          success: true,
          message: `${preamble}Found ${filtered.length} relevant document(s) (out of ${data.length} candidates):\n\n${lines.join("\n\n---\n\n")}`,
        };
      } catch (err) {
    ...
    case "create_hiring_candidate": {
      ...
      // Only include user ID fields if we have a valid UUID
    ...
    case "add_bid_details": {
      ...
      const specType = String(args.spec_type || "general").trim();
      if (!opportunityId) {
        return { success: false, error: "opportunity_id is required" };
      }

      // Build the requirement_details from all the fields the user provided
      const details = {};
      const fieldMap = {
        pier_count: "pier_count", pier_depth: "pier_depth", pier_diameter: "pier_diameter",
        estimated_days: "estimated_days", mob_days: "mob_days", crane_required: "crane_required",
        equipment_details: "equipment_details", soil_conditions: "soil_conditions",
        difficulty_factor: "difficulty_factor", mob_demob_cost: "mob_demob_cost",
        subcontractor_cost: "subcontractor_cost", notes: "notes",
      };
      for (const [key, dbKey] of Object.entries(fieldMap)) {
        if (args[key] !== undefined && args[key] !== null && args[key] !== "") {
          details[dbKey] = args[key];
        }
      }
      ...
      const row = {
        ...
        material_cost_estimate: args.material_cost_estimate || null,
        labor_cost_estimate: args.labor_cost_estimate || null,
        estimated_hours: args.estimated_days ? args.estimated_days * 10 : null,
        special_requirements: args.notes || null,
        similar_past_projects: [],
        risk_factors: [],
      };

      // Add difficulty-based risk factors
      if (args.difficulty_factor && args.difficulty_factor !== "standard") {
        row.risk_factors.push({ type: "difficulty", detail: `Job rated as ${args.difficulty_factor}` });
      }
      if (args.soil_conditions) {
        row.risk_factors.push({ type: "soil", detail: args.soil_conditions });
      }

      const { data, error } = await supabase
        .from("project_specifications")
        .insert(row)
        .select("id, spec_type, requirement_details, complexity_score")
        .single();

      if (error) return { success: false, error: error.message };

      // Also update opportunity with key fields if provided
      const oppUpdates = {};
      if (args.pier_count) oppUpdates.pier_count = args.pier_count;
      if (args.estimated_days) oppUpdates.estimated_duration = args.estimated_days;
      if (args.crane_required !== undefined) oppUpdates.crane_required = args.crane_required;
      if (Object.keys(oppUpdates).length) {
        await supabase.from("sales_opportunities").update(oppUpdates).eq("id", opportunityId);
      }

      const parts = [];
      if (args.pier_count) parts.push(`${args.pier_count} piers`);
      if (args.pier_depth) parts.push(`${args.pier_depth} depth`);
      if (args.pier_diameter) parts.push(`${args.pier_diameter} diameter`);
      if (args.estimated_days) parts.push(`${args.estimated_days} days`);
      if (args.material_cost_estimate) parts.push(`$${Number(args.material_cost_estimate).toLocaleString()} materials`);
      if (args.labor_cost_estimate) parts.push(`$${Number(args.labor_cost_estimate).toLocaleString()} labor`);

      return {
        success: true,
        message: `Saved ${specType} details for this bid${parts.length ? `: ${parts.join(", ")}` : ""}. This data will be used in the next bid analysis. Anyone else with more details can add them anytime.`,
      };
    }
...
/**
 * Local bid analysis — uses project specs, historical crew_jobs, competitors,
 * market intel, and client history to generate a recommendation.
 */
async function localBidAnalysis(supabase, opportunityId, args) {
  try {
    ...
    // Project specs attached to this opportunity (from add_bid_details)
    const { data: specs } = await supabase
      .from("project_specifications")
      .select("*")
      .eq("opportunity_id", opportunityId);

    // Historical data from completed crew_jobs
    const { data: completedJobs } = await supabase
      .from("crew_jobs")
      .select("job_name, bid_amount, contract_amount, estimated_days, actual_days, mob_days, actual_mob_days, pier_count, scope_description, customer_name, job_status")
    ...
    // ── 2. Extract details from specs ──

    let specPierCount = null;
    let specDays = null;
    let specMaterialCost = 0;
    let specLaborCost = 0;
    let specMobCost = 0;
    let specSubCost = 0;
    let specDifficulty = "standard";
    let specNotes = [];

    for (const spec of specs || []) {
      const d = spec.requirement_details || {};
      if (d.pier_count) specPierCount = d.pier_count;
      if (d.estimated_days) specDays = d.estimated_days;
      if (d.difficulty_factor) specDifficulty = d.difficulty_factor;
      if (d.notes) specNotes.push(d.notes);
      if (spec.material_cost_estimate) specMaterialCost += Number(spec.material_cost_estimate);
      if (spec.labor_cost_estimate) specLaborCost += Number(spec.labor_cost_estimate);
      if (d.mob_demob_cost) specMobCost += Number(d.mob_demob_cost);
      if (d.subcontractor_cost) specSubCost += Number(d.subcontractor_cost);
    }

    const pierCount = specPierCount || opp.pier_count || null;
    const estDays = specDays || opp.estimated_duration || null;

    // ── 3. Historical pricing from completed jobs ──

    const historicalInsights = { costPerPier: null, costPerDay: null, avgMargin: null, jobCount: 0 };
    const validJobs = (completedJobs || []).filter((j) => j.bid_amount > 0);
    historicalInsights.jobCount = validJobs.length;

    if (validJobs.length) {
      // Cost per pier from jobs that have pier counts
      const pierJobs = validJobs.filter((j) => j.pier_count > 0);
      if (pierJobs.length) {
        const totalCostPerPier = pierJobs.reduce((sum, j) => {
          const amount = j.contract_amount || j.bid_amount;
          return sum + amount / j.pier_count;
        }, 0);
        historicalInsights.costPerPier = Math.round(totalCostPerPier / pierJobs.length);
      }

      // Cost per day from jobs that have day counts
      const dayJobs = validJobs.filter((j) => (j.actual_days || j.estimated_days) > 0);
      if (dayJobs.length) {
        const totalCostPerDay = dayJobs.reduce((sum, j) => {
          const amount = j.contract_amount || j.bid_amount;
          const days = j.actual_days || j.estimated_days;
          return sum + amount / days;
        }, 0);
        historicalInsights.costPerDay = Math.round(totalCostPerDay / dayJobs.length);
      }

      // Average margin (bid vs contract)
      const marginJobs = validJobs.filter((j) => j.contract_amount > 0 && j.bid_amount > 0);
      if (marginJobs.length) {
        const totalMargin = marginJobs.reduce((sum, j) => {
          return sum + ((j.contract_amount - j.bid_amount) / j.bid_amount) * 100;
        }, 0);
        historicalInsights.avgMargin = Math.round(totalMargin / marginJobs.length * 100) / 100;
      }
    }

    // ── 4. Calculate base amount ──

    let baseAmount = Number(opp.value_estimate || 0);
    let baseSource = "value_estimate";
    const directCosts = specMaterialCost + specLaborCost + specMobCost + specSubCost;

    if (directCosts > 0) {
      // If we have actual cost estimates, use those as the base
      baseAmount = directCosts;
      baseSource = "itemized_costs";
    } else if (pierCount && historicalInsights.costPerPier) {
      // Estimate from historical cost-per-pier
      baseAmount = pierCount * historicalInsights.costPerPier;
      baseSource = "historical_cost_per_pier";
    } else if (estDays && historicalInsights.costPerDay) {
      // Estimate from historical cost-per-day
      baseAmount = estDays * historicalInsights.costPerDay;
      baseSource = "historical_cost_per_day";
    }

    if (!baseAmount) baseAmount = 100000;

    // ── 5. Margin calculation ──

    let avgMargin = 15.0;
    if (historicalInsights.avgMargin != null) avgMargin = historicalInsights.avgMargin;
    ...
    if (specDifficulty === "moderate") avgMargin += 2;
    else if (specDifficulty === "difficult") avgMargin += 5;
    else if (specDifficulty === "extreme") avgMargin += 8;
    ...
    if ((specs || []).length > 0) confidence = Math.min(0.95, confidence + 0.1);
    if (historicalInsights.jobCount > 5) confidence = Math.min(0.95, confidence + 0.05);
    if (directCosts > 0) confidence = Math.min(0.95, confidence + 0.1);
    ...
    if (!marketData.length && historicalInsights.jobCount < 3) { riskFactors.push("Limited pricing data"); if (riskLevel === "low") riskLevel = "medium"; }
    if (specDifficulty === "difficult" || specDifficulty === "extreme") { riskFactors.push(`${specDifficulty} job conditions`); if (riskLevel === "low") riskLevel = "medium"; }
    if (!(specs || []).length) { riskFactors.push("No detailed specs yet — analysis will improve as details are added"); }
    ...
    const reasoning = [];
    reasoning.push(`Base: ${baseSource === "itemized_costs" ? "itemized cost estimates" : baseSource === "historical_cost_per_pier" ? `historical avg $${historicalInsights.costPerPier?.toLocaleString()}/pier × ${pierCount} piers` : baseSource === "historical_cost_per_day" ? `historical avg $${historicalInsights.costPerDay?.toLocaleString()}/day × ${estDays} days` : "opportunity value estimate"}`);
    reasoning.push(`Margin: ${Math.round(avgMargin * 100) / 100}%`);
    if (competitorCount) reasoning.push(`${competitorCount} known competitors`);
    if (historicalInsights.jobCount) reasoning.push(`${historicalInsights.jobCount} completed jobs in historical data`);
    ...
    if (baseSource !== "value_estimate") {
      lines.push(``, `**Pricing basis:** ${baseSource === "itemized_costs" ? `Itemized costs: ${money(directCosts)}` : baseSource === "historical_cost_per_pier" ? `Historical avg ${money(historicalInsights.costPerPier)}/pier × ${pierCount} piers` : `Historical avg ${money(historicalInsights.costPerDay)}/day × ${estDays} days`}`);
    }

    if (directCosts > 0) {
      lines.push(``, `**Cost breakdown:**`);
      if (specMaterialCost) lines.push(`- Materials: ${money(specMaterialCost)}`);
      if (specLaborCost) lines.push(`- Labor: ${money(specLaborCost)}`);
      if (specMobCost) lines.push(`- Mob/demob: ${money(specMobCost)}`);
      if (specSubCost) lines.push(`- Subcontractors: ${money(specSubCost)}`);
      lines.push(`- Subtotal: ${money(directCosts)}`);
    }

    if (historicalInsights.jobCount > 0) {
      lines.push(``, `**Historical context:** ${historicalInsights.jobCount} completed jobs${historicalInsights.costPerPier ? `, avg ${money(historicalInsights.costPerPier)}/pier` : ""}${historicalInsights.costPerDay ? `, avg ${money(historicalInsights.costPerDay)}/day` : ""}`);
    }
