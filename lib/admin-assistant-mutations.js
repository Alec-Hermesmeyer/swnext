    ...
    case "search_knowledge_base": {
      ...
      try {
        ...
        const lines = filtered.map((d, i) => {
          ...
          // Trim content to budget; for structured DB data (JSON-like)
          // keep it tighter so the model doesn't get lost in raw fields.
          let content = d.content || "";
          const isStructured = content.trimStart().startsWith("{") || content.trimStart().startsWith("[");
          ...
          return `[Result ${i + 1} | ${category} | ${score}% match (${confidence})${section}${filename}${source}]\n${content}`;
        });
