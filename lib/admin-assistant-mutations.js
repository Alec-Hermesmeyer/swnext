  ...
  switch (name) {
    ...
    case "search_knowledge_base": {
      ...
      try {
        ...
        const matchCount = biddingFocus ? Math.max(RAG_MATCH_COUNT * 3, 24) : RAG_MATCH_COUNT;
        ...
        if (!data?.length) {
          return {
            ...
            message: biddingFocus
              ? "No relevant bid proposal chunks found. Ask an admin to run Knowledge Base → sync “Bid proposals (sales)” after uploads, then try again."
        ...
        if (biddingFocus) {
          ...
          if (bidRows.length) {
            ...
            preamble =
              "No bid-proposal chunks met the relevance threshold (category bidding). If proposals were uploaded recently, an admin may need to sync “Bid proposals (sales)” on the Knowledge Base page. Best general matches below:\n\n";
          }
        }
        ...
        if (!filtered.length) {
          return {
            success: true,
            message: biddingFocus
              ? "No sufficiently relevant bid documents found. Sync “Bid proposals (sales)” in the Knowledge Base after new uploads."
