  ...
  switch (name) {
    ...
    case "get_page_images": {
      try {
        ...
        const response = await fetch(`${siteUrl}/api/image-assignments`, { signal: controller.signal });
        clearTimeout(timeout);
        const data = await response.json();
        ...
        if (!entries.length) {
          return { success: true, message: "No custom image assignments saved yet. Pages are using the default images from the config." };
        }

        // Filter by page if specified
        const pageFilter = args.page ? String(args.page).toLowerCase() : null;
        const filtered = pageFilter
          ? entries.filter(([key]) => key.toLowerCase().startsWith(pageFilter))
          : entries;
        ...
        const lines = filtered.map(([key, url]) => {
    ...
    case "get_gallery_images": {
      try {
        ...
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 8000);
        const showAll = args.include_hidden ? "true" : "false";
        const response = await fetch(`${siteUrl}/api/gallery-images?all=${showAll}`, { signal: controller.signal });
        clearTimeout(timeout);
        const data = await response.json();
        const images = data.images || [];

        if (!images.length) {
          return { success: true, message: "No gallery images found." };
        }
        ...
        const lines = [`${totalVisible} visible, ${totalHidden} hidden across ${Object.keys(grouped).length} category/categories.\n`];
    ...
    case "toggle_gallery_image": {
      ...
      try {
        const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
        const response = await fetch(`${siteUrl}/api/gallery-images`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ id: image_id, is_visible: !!visible }),
        });
        if (!response.ok) {
          const data = await response.json();
          return { success: false, error: data.error || "Failed to update gallery image" };
        }
        return { success: true, message: `Gallery image ${visible ? "is now visible" : "has been hidden"} on the public gallery page.` };
      } catch (err) {
