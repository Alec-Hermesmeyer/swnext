...
export async function executeAdminAssistantMutation(supabase, name, args) {
  switch (name) {
    ...
    case "get_social_planning": {
      try {
        const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
        const response = await fetch(`${siteUrl}/api/social/planning/summary`);
        if (!response.ok) {
        ...
        return { success: false, error: `Could not fetch planning summary: ${err.message}` };
      }
    }
    ...
    case "get_social_library": {
      try {
        const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
        const response = await fetch(`${siteUrl}/api/social/library/posts`);
        if (!response.ok) {
        ...
        return { success: false, error: `Could not fetch post library: ${err.message}` };
      }
    }
    ...
    case "social_strategy_chat": {
      ...
      try {
        ...
        return { success: false, error: `Social strategy chat failed: ${err.message}` };
      }
    }
